...
def get_audio(video_path, directory_path):
    """
    Extracts the audio track from a video file, applies noise reduction, and saves the resulting audio file
    as a WAV file.
    ...
    directory_path : str
        The directory where the output audio file will be saved.
    ...
    -------
    IOError
        If the input video file cannot be read or the output audio file cannot be written.

    Dependencies:
    -------------
    This function requires the following external libraries to be installed:
        - moviepy
        - librosa
        - noisereduce
    ...
    Notes:
    ------
    This function uses the MoviePy library to extract the audio track from the video file and save it as a WAV file.
    It then loads the audio file using the librosa library, applies noise reduction using the noisereduce library,
    and saves the resulting audio file using the scipy library.
    """
    clip = mp.VideoFileClip(video_path)

    audio_path = video_path.replace('.mp4', '.wav')
    x = audio_path.split("/")
    audio_path = directory_path + '/' + x[-1]
    clip.audio.write_audiofile(audio_path)

    music, rate = lr.load(audio_path)
    reduced_noise = nr.reduce_noise(y=music, sr=rate)
    wavfile.write(audio_path, rate, reduced_noise)

def build_spectogram(audio_path, plot_path, bar = False):
    """
    Builds a mel-scaled spectrogram from an audio file, saves it as a PNG image file, and optionally adds a colorbar.
    ...
    plot_path : str
        The directory where the output PNG image file will be saved.
    bar : bool, optional
        Whether to include a colorbar in the output image (default is False).
    ...
    -------
    IOError
        If the input audio file cannot be read or the output image file cannot be written.
    ...
    Notes:
    ------
    This function uses the librosa library to compute a mel-scaled spectrogram from the input audio file.
    It then uses the matplotlib library to plot the spectrogram as a heatmap and save it as a PNG image file.
    If the `bar` parameter is set to True, the function also adds a colorbar to the image.
    """
    music, rate = lr.load(audio_path)
    ...
    a = x[-1].replace('.wav', '.png')
    plt.savefig(plot_path+'/'+a)


def get_audios_and_spectograms(directory_path, audio_folder, spectogram_folder):
    """
    Extracts the audio tracks and builds mel-scaled spectrograms for all the video files in a directory.

    Parameters:
    -----------
    directory_path : str
        The path to the directory containing the input video files.
    audio_folder : str
        The directory where the output audio files will be saved.
    spectogram_folder : str
        The directory where the output PNG image files will be saved.
    ...
    -------
    IOError
        If the input video files cannot be read or the output audio/image files cannot be written.

    Dependencies:
    -------------
    This function depends on the following two functions:
        - get_audio: extracts the audio track from a video file and saves it as a WAV file.
        - build_spectogram: builds a mel-scaled spectrogram from an audio file and saves it as a PNG image file.

    Notes:
    ------
    This function loops through all the files in the input directory and for each video file, it first extracts the
    audio track using the `get_audio` function and saves it as a WAV file in the `audio_folder`. It then builds a
    mel-scaled spectrogram from the WAV file using the `build_spectogram` function and saves it as a PNG image file
    in the `spectogram_folder`.
    """
    dir_list = os.listdir(directory_path)
    for item in dir_list:
        video_path = directory_path + '/' + item
        get_audio(video_path, audio_folder)

        item = item.replace('.mp4', '.wav')

        audio_path = audio_folder + '/' + item
        build_spectogram(audio_path, spectogram_folder)


def get_text_data(data_path, save_path):
    data = pd.read_json(data_path, orient='index')
    df = np.array([], dtype='object')
    df = df.reshape(-1,3)

    for item in os.listdir('data/utterances_final'):
        item = item.replace('.mp4', '')
        x = item.replace('_', '')
        x = int(x)
        df = np.append(df, [[item, data.loc[x]['utterance'], data.loc[x]['sarcasm']]], axis=0)

    ready_data = pd.DataFrame(df, columns=['clip no.', 'utterance', 'sarcasm'])
    ready_data.to_csv(save_path + '/ready_data.csv')
